...
class ResidualAttentionBlock_VPT_MoE(nn.Module):
    """
...
class ResidualAttentionBlock_IVLP_MoE(nn.Module):
    """
    Alignment-aware soft MoE for IVLP/VPT deep prompts.
    Key points:
      1. Configurable CLS/token-mean/hybrid routing
      2. Anchored residual fusion: base + alpha * (mixed - base)
      3. Bounded positive alpha/tau for stability
    """
    ...
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
        ...
        if i != 0:
            ...
            if self.add_prompt:
                ...
                self.base_prompt = nn.Parameter(base_prompt_tensor)
                self.expert_prompts = nn.Parameter(expert_prompts_tensor)
                self.gate = nn.Linear(d_model, self.num_experts)
    ...
    def _mix_expert_prompt(self, gate_source: torch.Tensor, orig_dtype: torch.dtype):
        gate_logits = self._router_logits(gate_source)
        tau = self._bounded_scale(self.tau, self.tau_min, self.tau_max)
        gate_weights = F.softmax(
            gate_logits.float() / tau, dim=-1
        )
        self._last_gate_weights = gate_weights

        expert_prompts = self.expert_prompts.to(orig_dtype).unsqueeze(0)
        weights = gate_weights.to(orig_dtype).unsqueeze(-1).unsqueeze(-1)
        mixed = (weights * expert_prompts).sum(dim=1)
        return mixed.permute(1, 0, 2).to(orig_dtype)

    def expert_diversity_loss(self):
        if not self.add_prompt:
            return self._zero_loss()
        num_experts = self.expert_prompts.shape[0]
        if num_experts <= 1:
            return self._zero_loss()
        flat = F.normalize(self.expert_prompts.reshape(num_experts, -1), dim=-1)
        similarity = flat @ flat.T
        mask = torch.triu(torch.ones(num_experts, num_experts, device=flat.device), diagonal=1).bool()
        return similarity[mask].clamp(min=0).mean()
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.add_prompt:
            if not self.text_layer:
                prefix = x[0:x.shape[0] - self.n_ctx_visual, :, :]
                base_prompt = self.base_prompt.to(orig_dtype).unsqueeze(0).expand(x.shape[1], -1, -1)
                base_prompt = base_prompt.permute(1, 0, 2)
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe_MoE(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        elif current_trainer == 'VPT_MoE':
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_VPT_MoE(width, heads, attn_mask, True,
                                                                         text_layer, i,
                                                                         design_details) if prompts_needed > i
                                             else ResidualAttentionBlock_VPT_MoE(width, heads, attn_mask, False,
                                                                              text_layer, i, design_details)
                                             for i in range(layers)])
        elif current_trainer == 'IVLP_MoE':
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe" or trainer == "MaPLe_MoE":
