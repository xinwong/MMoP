...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'MaPLe_MoE',
                      "vision_depth": 0,
                      "language_depth": 0, "vision_ctx": 0,
                      "language_ctx": 0,
                      "maple_length": cfg.TRAINER.MoEAdvMaPLe.N_CTX,
                      "num_experts": cfg.TRAINER.MoEAdvMaPLe.NUM_EXPERTS
                      }
...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.MoEAdvMaPLe.N_CTX
        ctx_init = cfg.TRAINER.MoEAdvMaPLe.CTX_INIT
        dtype = clip_model.dtype
        ctx_dim = clip_model.ln_final.weight.shape[0]
        clip_imsize = clip_model.visual.input_resolution
        cfg_imsize = cfg.INPUT.SIZE[0]
        # Default is 1, which is compound shallow prompting
        assert cfg.TRAINER.MoEAdvMaPLe.PROMPT_DEPTH >= 1, "For AdvMaPLe, PROMPT_DEPTH should be >= 1"
        self.compound_prompts_depth = cfg.TRAINER.MoEAdvMaPLe.PROMPT_DEPTH  # max=12, but will create 11 such shared prompts
...
class MoEAdvMaPLe(TrainerX):
    ...
    def check_cfg(self, cfg):
        assert cfg.TRAINER.MoEAdvMaPLe.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.MoEAdvMaPLe.PREC == "fp32" or cfg.TRAINER.MoEAdvMaPLe.PREC == "amp":
        ...
        self.scaler = GradScaler() if cfg.TRAINER.MoEAdvMaPLe.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.MoEAdvMaPLe.PREC
        if prec == "amp":
            with autocast():
                if loss_type == "trades":
